...
char* get_var_class(char *varname) {
    VarClass *v;
    for(v = var_classes; v; v = v->next) if(strcmp(v->varname, varname) == 0) return v->classname;
    return nil;
}
...
char* map_type(char *t) {
    int len; Node *n;
    static char buf[8][128]; static int bi = 0; char *b = buf[bi++ % 8];
...
%left TEQ
%left TCHANSEND TCHANRECV TCHANTRY
%left TOR
%left TAND
%left '|'
%left '^'
...
void gen_class_header(Node *c) { Node *m; print("typedef struct %s_Client {\n\tint fd;\n\tvoid *shm_base;\n\tvoid *table;\n\tlong ref;\n\tvoid *dispatch_chan;\n\tint distance;\n", c->name);
    for(m = c->left; m; m = m->next) if(m->type == NInherit) print("\t%s_Client;\n", m->name);
    print("} %s_Client;\n\n", c->name); }
void gen_class_server(Node *c) {
    Node *m;
    print("typedef struct %s_Internal %s_Internal;\nstruct %s_Internal {\n\tlong ref;\n", c->name, c->name, c->name);
    for(m = c->left; m; m = m->next){
        if(m->type == NInherit) print("\t%s_Internal;\n", m->name);
        if(m->type == NProp || m->type == NState) print("\t%s %s;\n", map_type(m->typename), m->name);
    }
    print("};\n\n");
}
void codegen(Node *root) {
    ...
    for(n = root; n; n = n->next) if(n->type == NClass) gen_class_server(n);
...
static void scan_buffer(char *buf, long len) {
    ...
    while(pos < len){
        ...
        if(isalpha(c) || c == '_'){
            ...
            if(strcmp(name, "class") == 0 || strcmp(name, "struct") == 0 || strcmp(name, "interface") == 0){
