...
# BASE CONTENT (from o9_clean.y but updated)
content = """%{
#include <u.h>
#include <libc.h>
#include <bio.h>
...
VarClass *var_classes;

void
add_var_class(char *varname, char *classname)
{
    VarClass *v = malloc(sizeof(VarClass));
...
{
    VarClass *v;
    for(v = var_classes; v; v = v->next)
        if(strcmp(v->varname, varname) == 0) return v->classname;
    return nil;
}
...
type_cast(char *t)
{
    ...
    if(strcmp(t, "vlong") == 0) return "vlong";
    return "vlong";
}
...
is_primitive(char *t)
{
    ...
    if(strcmp(t, "int64") == 0 || strcmp(t, "uint64") == 0 ||
       ...
       strcmp(t, "void") == 0 || strcmp(t, "chan") == 0) return 1;
...
func_decl:
    ...
    {
        $$ = mk(NMethod, $7->name, $11->name, $13, $9);
    }
    ;
...
param:
    typename TIDENT { $$ = mk(NProp, $2->name, $1->name, nil, nil); }
    ;
...
stmt:
    ...
    | TIF '(' expr ')' '{' stmt_list '}' else_clause
    {
    ...
    {
        $$ = mk(NWhile, nil, nil, $3, $6);
    }
    | TFOR '(' for_init for_cond for_step ')' '{' stmt_list '}'
    {
        Node *f = mk(NFor, nil, nil, $3, mk(0, nil, nil, $4, $5));
        f->right->next = $8;
        $$ = f;
    }
    ;
...
expr:
    ...
    | TNIL { $$ = mk(NBoolLit, "0", "vlong", nil, nil); }
    | TIDENT { $$ = $1; }
...
static int in_prescan;
char *loaded_files[64];
int num_loaded_files = 0;

static int for_paren_depth = -1;
static int pushback[8];
static int npush = 0;
...
yylex(void)
{
    ...
    while((c = lex_getc()) != Beof){
        ...
        if(isdigit(c)){
            ...
            int i = 0;
            buf[i++] = c;
            while(isdigit(c = lex_getc())) if(i < 63) buf[i++] = c;
            lex_ungetc(c);
            buf[i] = '\\0';
            yylval.name = strdup(buf);
            return TINTLIT;
        ...
        if(isalpha(c) || c == '_'){
            ...
            int i = 0;
            buf[i++] = c;
            while(isalnum(c = lex_getc()) || c == '_') if(i < 63) buf[i++] = c;
            lex_ungetc(c);
            buf[i] = '\\0';
            ...
            if(strcmp(buf, "int64") == 0 || strcmp(buf, "string") == 0 || strcmp(buf, "bool") == 0 || strcmp(buf, "void") == 0) return TTYPE;
            if(find_class(buf)) return TTYPE;
            return TIDENT;
        }
...
void mark_locals(Node *n);
char *local_vars[128];
int num_locals = 0;
int in_method_body = 0;
int has_return = 0;
int in_class_context = 0;
...
mark_locals(Node *n)
{
    ...
    for(c = n; c; c = c->next){
        if(c->type == NLocalVar){
            if(num_locals < 128) local_vars[num_locals++] = c->name;
        }
        mark_locals(c->left);
        mark_locals(c->right);
    }
...
gen_expr(Node *e)
{
    ...
    case NBoolLit:
        ...
        break;
    case NIdent:
        if(is_local(e->name) || !in_class_context)
            print("%s", e->name);
        else
            print("self->%s", e->name);
        break;
    case NMsgSend:
        {
            ...
            if(strncmp(lt, "List:", 5) == 0){
                ...
                if(strcmp(e->name, "Clear") == 0){
                    print("(vlong)("); gen_expr(e->left); print(".len = 0)");
                    break;
                }
            }
        ...
        print("(vlong)obj9_msgSend(&"); gen_expr(e->left); print(", \\"%s\\", 0x%lux, nil)", e->name, o9_hash(e->name));
        break;
    case NPropRead:
        {
    ...
    case NFuncCall:
        if(strcmp(e->name, "print") == 0){
            print("fprint(1, ");
            Node *a = e->left;
            if(a && a->type == NStringLit){
                print("\\"%s\\"", a->name);
                for(a = a->next; a; a = a->next){ print(", "); gen_expr(a); }
            }
            print(")");
        }
        break;
    ...
    case NAssign:
        if(e->left && e->left->type == NArrayGet){
            ...
            if(strncmp(lt, "Dict:", 5) == 0){
                print("o9_dict_set(&"); gen_expr(e->left->left); print(", "); gen_expr(e->left->right); print(", (void*)("); gen_expr(e->right); print("))");
            } else if(strncmp(lt, "List:", 5) == 0){
                char *et = lt + 5;
                print("({ %s __v = ", map_type(et)); gen_expr(e->right); print("; o9_slice_set(&"); gen_expr(e->left->left); print(", "); gen_expr(e->left->right); print(", &__v); })");
            }
            break;
        }
        gen_expr(e->left); print(" = "); gen_expr(e->right);
        break;
    }
}
...
gen_struct_def(Node *c)
{
    ...
    for(m = c->left; m; m = m->next) if(m->type == NProp) print("\\t%s %s;\\n", map_type(m->typename), m->name);
    print("};\\n\\n");
}
...
{
    print("typedef struct %s_Client { int distance; void *shm_base; } %s_Client;\\n", c->name, c->name);
}
...
codegen(Node *root)
{
    ...
    for(n = root; n; n = n->next){
        if(n->type == NMethod && strcmp(n->name, "main") == 0){
            print("void main(int argc, char **argv) {\\n\\tUSED(argc); USED(argv);\\n");
            num_locals = 0; mark_locals(n->left);
            for(n = n->left; n; n = n->next) gen_stmt(nil, n);
            print("}\\n");
        }
    }
}

static void scan_buffer(char *buf, long len) { /* Simplified scan */ }
static void scan_file(char *path) { /* Simplified scan */ }
static void prescan(void) { /* Simplified prescan */ }

static int member_exists(Node *cnode, char *name) {
    Node *m;
    for(m = cnode->left; m; m = m->next) if(m->name && strcmp(m->name, name) == 0) return m->type;
    return -1;
}

Node *cur_meth; char *tc_class;
static void typecheck_expr(Node *e, int *errs) { /* Simplified for now */ }
static void check_node(Node *n, int *errs) {
    Node *c;
    for(c = n; c; c = c->next) { typecheck_expr(c, errs); check_node(c->left, errs); check_node(c->right, errs); }
}
static int typecheck(Node *root) { int e = 0; clear_type_syms(); check_node(root, &e); return e; }

int main(int argc, char **argv) {
    input_buf = malloc(8192);
    input_len = read(0, input_buf, 8192);
    if(yyparse() == 0) { if(typecheck(ast_root) == 0) codegen(ast_root); }
    return 0;
}
"""
